...
void signUp(struct Pengguna* pengguna, int* jumlahPengguna) {
    ...
    if (file != NULL) {
        fprintf(file, "%s %s\n", pengguna[*jumlahPengguna - 1].username, pengguna[*jumlahPengguna - 1].password);
        fclose(file);
    } else {
...
void tulisDataPengguna(struct Pengguna* pengguna, int jumlahPengguna) {
    FILE* file = fopen("data_pengguna.txt", "w");
    int i;
    if (file != NULL) {
        for (i = 0; i < jumlahPengguna; i++) {
            fprintf(file, "%s %s\n", pengguna[i].username, pengguna[i].password);
        }
        fclose(file);
    } else {
        printf("Gagal menulis data ke dalam file.\n");
    }
}
    ...
    do {
        ...
        switch (pilihan) {
            ...
            case 2:
				...
				tulisDataPengguna(dataPengguna, jumlahPengguna); }
                break;
            case 0:
