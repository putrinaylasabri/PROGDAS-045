...
void tambahPeminjaman (const char *namaPeminjam, const char *judulBuku, const char *tanggalPinjam, const char *tanggalKembali, const char *statusKembali);
...
bool cekKetersediaanBuku(const char *judulCari) {
    ...
    while (fscanf(file, "%d %d %d %s %s %s\n", &temp.kode, &temp.tahun, &temp.jumlahBuku, temp.judul, temp.pengarang, temp.genre) == 6) {
        ...
        if (strstr(judulTemp, judulCari) != NULL && temp.jumlahBuku > 0) {
            fclose(file);
            return true;
        }
    }
...
void peminjamanBuku(char *breadcrumb) {
    ...
    // Cek ketersediaan buku sebelum melakukan peminjaman
    // Assuming you have a function cekKetersediaanBuku() defined elsewhere
    ...
    const char statusDipinjam[] = "DIPINJAM";
    tambahPeminjaman(pinjaman.namaPeminjam, pinjaman.judulBuku, pinjaman.tanggalPinjam, pinjaman.tanggalKembali, statusDipinjam);
...
void tambahPeminjaman(const char *namaPeminjam, const char *judulBuku, const char *tanggalPinjam, const char *tanggalKembali, const char *statusKembali) {
    ...
    fprintf(file, "%s|%s|%s|%s|%s\n", namaPeminjam, judulBuku, tanggalPinjam, tanggalKembali, statusKembali);
    fclose(file);
}
...
void pengembalianBuku(char *breadcrumb) {
    ...
    // Update the status to "KEMBALI"
    tambahPeminjaman(pinjaman.namaPeminjam, pinjaman.judulBuku, pinjaman.tanggalPinjam, pinjaman.tanggalKembali, "KEMBALI");
...
void hitungDenda(const char *tanggalKembali) {
    ...
    time_t selisihDetik = difftime(mktime(&t1), mktime(&t2));

    // Check if it's negative, meaning the book is returned before the due date
    if (selisihDetik > 0) {
        int selisihHari = (int)(selisihDetik / (60 * 60 * 24));
...
void historyPeminjaman(char *breadcrumb) {
    ...
    printf("\n");
    printf("\n============================================================================================================================================================\n");
    printf("| %-30s | %-40s | %-20s | %-20s | %-20s\n", "Nama Peminjam", "Judul Buku", "Tanggal Pinjam", "Tanggal Kembali", "Status Pengembalian");
    printf("============================================================================================================================================================\n");

    while (fscanf(filePeminjaman, "%[^|]|%[^|]|%[^|]|%[^|]|%[^\n]\n", pinjaman.namaPeminjam, pinjaman.judulBuku, pinjaman.tanggalPinjam, pinjaman.tanggalKembali, pinjaman.statusKembali) == 5) {
        printf("| %-30s | %-40s | %-20s | %-20s | %-20s\n",
               pinjaman.namaPeminjam, pinjaman.judulBuku, pinjaman.tanggalPinjam, pinjaman.tanggalKembali, pinjaman.statusKembali);
    }
